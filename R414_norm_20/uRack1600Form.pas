...
{$REGION 'ќбработчик TRack1600Form.tmrMainTimer(Sender: TObject)'}
...
{$REGION '—мещение формы влево/вправо, чтобы не перекрывать волномер'}
...
begin
  if Ord(Key) = 27 then
  begin
    //»нициаци€ событи€ "«акрытие окна стойки"
...
{$REGION 'ѕрокрутка изображени€ стойки колесом мыши'}
  ...
  case CurFormId of
    idRack1600A:
      begin
        ...
        begin
          if not(InputQuery('¬ведите значение волны', PName, strDigit)) then
            Exit;
